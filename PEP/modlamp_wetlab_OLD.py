...
class CD:
    ...
    def calc_molar_ellipticity(self):
        ...
        for i, data in enumerate(self.circular_dichroism):
            ...
            mol_ellipt = (data[:, 1] * self.mw[i]) / (self.conc_mgml[i] * float(self.pathlen))
    ...
    def calc_meanres_ellipticity(self):
        ...
        for i, data in enumerate(self.circular_dichroism):
            ...
            mol_ellipt = (data[:, 1] * self.meanres_mw[i]) / (self.conc_mgml[i] * float(self.pathlen))
    ...
    def _plot_all(self, data, w, y_lim, Workdirpath, htmlOutDir, htmlFname):
        """Private plot function used by :py:func:`modlamp.wetlab.CD.plot()` for plotting combined CD plots"""
        ...
        plt.savefig(os.path.join(Workdirpath, htmlOutDir, "all.pdf"), dpi=150)
    ...
    def _check_datatype(self, data, i, comb_flag):
        ...
        if data == 'mean residue ellipticity':
            print self.meanres_ellipticity[i]
    ...
    def plot(self, Workdirpath, htmlOutDir, htmlFname, data='mean residue ellipticity', combine='solvent', ylim=None, ):
        ...
        if data in ['mean residue ellipticity', 'molar ellipticity', 'circular dichroism']:
            ...
            if combine == 'all':
                self._plot_all(data, w, ylim, Workdirpath, htmlOutDir, htmlFname)
...
if __name__=="__main__":
    ...
    cd = CD(args.InFiles, args.Wmin, args.Wmax, args.amide, args.pathlen)

    if args.Method == 'calc_molar_ellipticity':
        cd.calc_molar_ellipticity()
        cd.plot(args.Workdirpath, args.htmlOutDir, args.htmlFname, data='mean residue ellipticity', combine='solvent')

    elif args.Method == 'calc_meanres_ellipticity':
        cd.calc_meanres_ellipticity()
        cd.plot(args.Workdirpath, args.htmlOutDir, args.htmlFname, data='mean residue ellipticity', combine='solvent')
